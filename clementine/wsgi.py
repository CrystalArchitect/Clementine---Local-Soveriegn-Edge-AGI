"""
WSGI entry point, for running her under a real server (gunicorn) instead of
Flask's development server.

    gunicorn --workers 1 --threads 4 --bind 127.0.0.1:5000 wsgi:app

**One worker, deliberately.** Her memory lives in Python objects that are
saved to disk after each turn. Two workers would each hold their own copy and
overwrite each other's — she would forget things at random, which is the one
failure this project cannot tolerate. Threads are fine: they share the same
object. If she ever needs to serve more than one person at a time, that is a
...
Configuration comes from the environment, since there is no argv here:
    ...
    CLEM_MEMORY_DIR   her memory folder   (default clementine_memory)
